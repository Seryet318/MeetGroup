    ...
    private fun signInGoogle(account: GoogleSignInAccount){
        ...
        auth.signInWithCredential(credential)
            .addOnCompleteListener(this){
                if(it.isSuccessful){
                    val accountInfo = Account(auth.currentUser!!.displayName?.split(" ")?.get(0)!!,auth.currentUser!!.displayName?.split(" ")?.get(1)!!,auth.currentUser!!.email.toString())


                   val database = FirebaseDatabase.getInstance().getReference("Users").child(auth.currentUser!!.uid)
                    database.setValue(accountInfo).addOnCompleteListener(this){ task ->
